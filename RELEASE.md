...
- **Drifted completer baseline** (`Tests\CompleterBaseline.clixml`) is expected when Orch1 / OrchTest tenants gain or lose resources between releases — rerun `Tests\Capture-CompleterBaseline.ps1` if the Completer tests fail for a candidate-count diff. See the test output for the specific drift.
