...
## SYNOPSIS
Identity サーバーのロボットアカウントを新規作成/更新します。
...
## DESCRIPTION
Identity サーバーのロボットアカウントが、どのグループに所属するかを更新します。もし、存在しないロボットアカウント名を指定したときは、その名前で新規にロボットアカウントを作成し、このアカウントを指定のグループに追加します。
...
CSV ファイルをインポートすることにより、ロボットアカウントを作成/更新することもできます。インポートできる形式の CSV ファイルを生成するには、`Get-OrchPmRobot -ExportCsv` を実行します。この CSV ファイルをインポートする方法は、Example 2 を参照してください。
...
### Example 1
```powershell
PS Orch1:\> Set-OrchPmRobotAccount RobotAccountName MyGroup,YourGroup
```
...
### Example 2
```powershell
PS Orch1:\> Import-Csv c:ExportedPmRobotAccount.csv | Set-OrchPmRobotAccount
```

CSV ファイルをインポートして、Identity サーバーのロボットアカウントを新規作成もしくは更新します。インポートできる形式の CSV ファイルを生成するには、`Get-OrchPmRobot -ExportCsv` を実行します。カレントドライブよりも、CSV ファイルに記載された Path 列が優先されることに注意してください。

### Example 3
```powershell
PS Orch1:\> Import-Csv c:ExportedPmRobotAccount.csv | Set-OrchPmRobotAccount -Path .
```
