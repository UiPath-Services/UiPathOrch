...
## DESCRIPTION
Copy-PmExternalApplication コマンドレットは、UiPath Platform Management を使用してソース組織から宛先組織にPlatform Management外部アプリケーションをコピーします。このコマンドレットは、認証設定、権限、グループ関連付けを含む外部アプリケーション構成のコピーを作成し、複数のUiPath組織間での外部アプリケーション管理を可能にします。
...
プライマリエンドポイント: POST /api/ExternalClient
