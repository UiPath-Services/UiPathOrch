...
## DESCRIPTION
Outputs information about machines with specified names that are assigned to the target folders.
...
If no machine names are specified, it outputs all machines assigned to the target folders.
...
Displays all machines assigned to the current folder and all its subfolders.
When run in the root folder, it shows all machines assigned across all folders in that tenant.
...
Displays machines with the names specified assigned to the current folder and its subfolders.
This is useful for identifying which folders contain machines with a specified names.
You can specify multiple machine names, separated by commas, including wildcards.
Machine names can be auto-completed with \[Ctrl+Space\] or \[Tab\].
...
Displays machines with the names specified assigned to the specified folders.
You can specify multiple folders, separated by commas, including wildcards.
...
Displays output with only the selected columns.
Specify multiple columns separated by commas, including wildcards.
Column names can be auto-completed with \[Ctrl+Space\] or \[Tab\].
...
The CSV file will be located at the current location of the C: drive.
You can customize the CSV format by combining with \`select\` to specify which columns to include.
Try \`ii c:' to open the current location of the C: drive.
