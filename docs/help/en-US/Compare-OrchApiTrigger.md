...
HelpUri: https://github.com/UiPath-Services/UiPathOrch/blob/master/docs/help/en-US/Compare-OrchApiTrigger.md
Locale: en-US
Module Name: UiPathOrch
ms.date: 06/07/2026
PlatyPS schema version: 2024-05-01
title: Compare-OrchApiTrigger
---
...
Compares API (HTTP) triggers between two folders or Orchestrator instances and reports the differences.
...
```
Compare-OrchApiTrigger [-Path <string>] [-LiteralPath <string>] [-Recurse] [-Depth <uint>]
...
Compares API (HTTP) triggers between a reference location (-Path, or the current folder) and a difference location (-DifferencePath), resolved over Orchestrator drives and folders. The primary use is migration verification.

Triggers are matched by Name and these properties are compared: Enabled, Method, CallingMode, CallbackMode, AllowInsecureSsl, RunAsCaller, SuccessCallbackUrl, FailureCallbackUrl, JobPriority, RuntimeType, InputArguments, Description, and Tags. The signing Secret and the auto-generated Slug are not compared.
...
```powershell
PS C:\> Compare-OrchApiTrigger -Path Orch1:\Shared -DifferencePath Orch2:\Shared -Recurse
```

Walks Shared and all subfolders and reports the differing API triggers.

### Example 3: Keep the comparison as a migration report

```powershell
PS C:\> Compare-OrchApiTrigger * Orch2:\Shared -Path Orch1:\Shared -Recurse -IncludeEqual -ExportCsv report.csv
```
...
Writes the comparison to the specified CSV file path instead of to the pipeline. The columns are SideIndicator, Name, DifferenceName, Path, DifferencePath, and Differences — the same six the console shows, with Differences carrying the per-property "Prop: 'ref' => 'diff'" text. ReferenceObject and DifferenceObject are not exported: they are whole entities kept for downstream piping, and a CSV can hold nothing of them but their type name. A directory path writes ComparedApiTriggers.csv inside it. Requires a filesystem path (not an Orch: drive path).
...
Restricts the comparison to the named properties. Valid names: Enabled, Method, CallingMode, CallbackMode, AllowInsecureSsl, RunAsCaller, SuccessCallbackUrl, FailureCallbackUrl, JobPriority, RuntimeType, InputArguments, Description, Tags. Unrecognized names are warned about and ignored.
...
Returns one comparison record per API trigger, with SideIndicator, Name, the single-sided Path and DifferencePath, the per-property Differences (on "<>" rows), and the underlying ReferenceObject / DifferenceObject.

## NOTES

Triggers are matched by Name, case-insensitively. The signing Secret is never compared. This cmdlet is read-only and does not support -WhatIf / -Confirm.
