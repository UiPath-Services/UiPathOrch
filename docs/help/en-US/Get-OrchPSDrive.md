...
- **ProductVersion** — The Orchestrator product version reported by `/api/Status/Version` (e.g. `26.3.0-s195.3951`), shown in the default table between `Root` and `ApiVer`. Populated for drives that already hold an active sign-in (a cheap read on the existing token), by `Get-OrchProductVersion`, or by this cmdlet's `-Force` switch. A plain `Get-OrchPSDrive` never triggers an interactive sign-in, so cold (not-yet-authenticated) drives show an empty value.
