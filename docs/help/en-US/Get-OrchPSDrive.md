...
- **ProductVersion** — The Orchestrator product version reported by `/api/Status/Version` (e.g. `26.3.0-s195.3951`), shown in the default table between `Root` and `ApiVer`. This is a passive, cached read: it is populated only after the version has been fetched — by `Get-OrchProductVersion` or by this cmdlet's own `-Force` switch — so a plain `Get-OrchPSDrive` that lists every drive never triggers a network call or an interactive sign-in. Drives whose version has not been fetched show an empty value.
