...
namespace UiPath.PowerShell.Commands
{
    ...
    public class EnableMaintenanceModeCommand : OrchestratorPSCmdlet
    {
                ...
                IDictionary fakeBoundParameters)
            {
                ...
                foreach (var result in results)
                {
                    ...
                    foreach (var session in entities!
                        ...
                        .Where(session => session.MaintenanceMode == "Default")
            ...
            foreach (var drive in drives)
            {
                try
                {
                        ...
                        .OrderBy(session => session.SessionId))
                    {
                        ...
                        if (ShouldProcess(target, "Enable MaintenanceMode"))
                        {
                            try
                            {
                                drive.OrchAPISession.SetMaintenanceMode(session.SessionId, true, Force.IsPresent);
                                drive._dicMachineSessionRuntimes = null;
                            }
                            catch (Exception ex)
                            {
                                WriteError(new ErrorRecord(new OrchException(target, ex), "EnableMaintenanceModeError", ErrorCategory.InvalidOperation, session));
                            }
                        }
                    }
                }
