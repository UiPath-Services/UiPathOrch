...
namespace UiPath.PowerShell.Commands
{
    ...
    public class DisableLicenseRuntimeCommand : OrchestratorPSCmdlet
    {
                ...
                IDictionary fakeBoundParameters)
            {
                ...
                Parallel.ForEach(drives, drive =>
                {
                    ...
                    else
                    {
                        ...
                        nonCachedRobotTypes = specifiedRobotTypes
                            .Where(rt => !drive._dicLicenseRuntime?.ContainsKey(rt) ?? false);
                    }
            ...
            foreach (var drive in drives)
            {
                foreach (var robotType in specifiedRobotType)
                {
                        ...
                        .FilterByWildcards(l => l?.Key, wpKey))
                    {
                        ...
                        if (ShouldProcess(target, "Disable LicenseRuntime " + robotType))
                        {
                            try
                            {
                                drive.OrchAPISession.ToggleLicenseRuntime(robotType, license.Key!, license.MachineName!, false);
                                drive._dicLicenseRuntime?.TryRemove(robotType, out _);
                            }
                            catch (Exception ex)
                            {
                                WriteError(new ErrorRecord(new OrchException(target, ex), "DisableLicenseRuntimeError", ErrorCategory.InvalidOperation, license));
                            }
                        }
                    }
                }
