...
namespace UiPath.PowerShell.Commands
{
    ...
    public class DisableTriggerCommand : OrchestratorPSCmdlet
    {
            ...
            foreach (var (drive, folder) in drivesFolders)
            {
                try
                {
                        ...
                        .OrderBy(t => t.Name))
                    {
                        ...
                        if (ShouldProcess(trigger.GetPSPath(), "Disable Trigger"))
                        {
                            try
                            {
                                drive.OrchAPISession.EnableProcessSchedule(folder.Id ?? 0, [trigger.Id ?? 0], false);
                            ...
                            {
                                WriteError(new ErrorRecord(new OrchException(trigger.GetPSPath(), ex), "DisableTriggerError", ErrorCategory.InvalidOperation, trigger));
                            }
                        }
                    }
                }
