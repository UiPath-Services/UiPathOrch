    ...
    public class SetAssetCommand : OrchestratorPSCmdlet
    {
                ...
                IDictionary fakeBoundParameters)
            {
                ...
                foreach (var result in results)
                {
                    ...
                    foreach (var asset in entities!
                        ...
                        .FilterByWildcards(a => a?.Name, wpName))
                    {
                        if ((wpUserName != null && wpUserName.Count != 0) || (wpMachineName != null && wpMachineName.Count != 0))
                        {
            ...
            IEnumerable<ExtendedMachine?> specifiedMachines)
        {
            ...
            if (asset == null)
            {
                ...
                else
                {
                    ...
                    if (asset.UserValues != null && asset.UserValues.Any())
                    {
            ...
            {
                if (specifiedMachines != null && specifiedMachines.Any(m => m != null))
                {
                    // マシンを無視する旨の警告
        ...
        protected void BuildAssetDataFromParameterSets()
        {
            ...
            foreach (var param in parameters)
            {
                ...
                {
                    string target;
                    if (param.Path != null && param.Path.Any())
                    {
                ...
                if (param.UserName != null && param.UserName.Any(un => !string.IsNullOrEmpty(un)))
                    wpUserName = param.UserName.ConvertToWildcardPatternList();

                if (param.MachineName != null && param.MachineName.Any(mn => !string.IsNullOrEmpty(mn)))
                ...
                foreach (var (drive, folder) in drivesFolders)
                {
                    ...
                    if (specifiedMachines == null || !specifiedMachines.Any())
                    {
                    ...
                    foreach (var name in param.Name!)
                    {
                        ...
                        if (matchingAssets.Any())
                        {
                            ...
                            foreach (var matchingAsset in matchingAssets)
                            {
                                ...
                                if (asset != null && !parameterSets.Contains(asset))
                                    parameterSets.Add(asset);
                            }
                        }
        ...
        protected override void EndProcessing()
        {
            ...
            try
            {
                ...
                foreach (var asset in parameterSets)
                {
                    ...
                    try
                    {
                        if (existingAsset == null) // アセットを新規追加
                        {
                            if (!asset.HasDefaultValue.GetValueOrDefault() && (asset.UserValues == null || !asset.UserValues.Any()))
                            {
                                continue;
                            }
                                ...
                                if (ShouldProcess(target, "Update Asset"))
                                {
                                    ...
                                    try
                                    {
                                        if (asset.UserValues == null || asset.UserValues.Count() == 0)
                                        {
                                            ...
                                            if (sharedFolders != null && sharedFolders.AccessibleFolders != null)
                                            {
