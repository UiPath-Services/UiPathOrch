...
{
    [Cmdlet(VerbsLifecycle.Enable, "OrchTestSetSchedule", SupportsShouldProcess = true)]
    public class EnableTestSetScheduleCommand : OrchestratorPSCmdlet
    {
            ...
            foreach (var (drive, folder) in drivesFolders)
            {
                try
                {
                        ...
                        .OrderBy(ts => ts.Name))
                    {
                        ...
                        if (ShouldProcess(target, "Enable TestSetSchedule"))
                        {
                            try
                            {
                                drive.OrchAPISession.EnableTestSetSchedules(folder.Id ?? 0, true, [schedule.Id ?? 0]);
                                drive._dicTestSetSchedules?.TryRemove(folder.Id ?? 0, out _);
                            }
                            catch (Exception ex)
                            {
                                WriteError(new ErrorRecord(new OrchException(target, ex), "EnableTestSetScheduleError", ErrorCategory.InvalidOperation, schedule));
                            }
                        }
                    }
                }
