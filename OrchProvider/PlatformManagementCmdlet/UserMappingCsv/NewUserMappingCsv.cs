            ...
            ProgressReporter reporter, CancellationToken cancelToken)
        {
            ...
            foreach (var result in results)
            {
                ...
                try
                {
                    ...
                    if (detailedSrcGroup == null || detailedSrcGroup.members == null) continue;
            ...
            ProgressReporter reporter, CancellationToken cancelToken)
        {
            ...
            foreach (var result in results)
            {
                ...
                try
                {
                    ...
                    if (folderUsers == null) continue;

                    foreach (var folderUser in folderUsers.Where(u => u.UserEntity?.Type == "DirectoryUser"))
                    {
                        cancelToken.ThrowIfCancellationRequested();

                        if (folderUser.UserEntity == null || string.IsNullOrEmpty(folderUser.UserEntity.UserName)) continue;
