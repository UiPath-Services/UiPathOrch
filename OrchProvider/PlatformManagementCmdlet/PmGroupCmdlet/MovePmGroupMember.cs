    ...
    public class MoveOrchPmGroupMemberCommand : OrchestratorPSCmdlet
    {
        ...
        protected override void EndProcessing()
        {
            ...
            foreach (var update in updates.OrderBy(u => u.Key.drive.Name).ThenBy(u => u.Key.group.name))
            {
                ...
                if (command.directoryUserIDsToAdd != null && command.directoryUserIDsToRemove != null)
                {
                ...
                if (command.directoryUserIDsToAdd != null && existingMemberIds  != null)
                {
                    command.directoryUserIDsToAdd.RemoveAll(m => existingMemberIds.Contains(m));
                }

                // すでにグループにない id は、remove から削除しておく
                if (command.directoryUserIDsToRemove!= null && existingMemberIds != null)
                {
                ...
                if (command.directoryUserIDsToAdd    == null &&
                    command.directoryUserIDsToRemove == null)
                {
                    continue;
                }
