// Embedded Windows time-zone table (Id + DisplayName), generated from
// TimeZoneInfo.GetSystemTimeZones() on Windows. Orchestrator's -TimeZoneId
// requires a WINDOWS time-zone id (it rejects IANA ids such as Asia/Tokyo
// with "not a valid windows time zone id"). On Linux/macOS the OS time-zone
// database is IANA, so TimeZoneIdCompleter cannot derive these ids at runtime
// there — it uses this table instead, so cross-platform users still get
// Orchestrator-valid completions. Windows hosts keep using the live OS list.
// Regenerate at release time if the Windows zone set changes.
namespace UiPath.PowerShell.Core;

internal static class WindowsTimeZones
{
    internal static readonly (string Id, string DisplayName)[] All =
    [
