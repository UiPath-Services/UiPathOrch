...
public partial class OrchAPISession : IDisposable
{
    ...
    // PIMS (Process Instance Management Service) backs the Maestro product.
    // - Endpoint:     {tenant}/pims_/api/v1/instances
    // - List is tenant-wide; folder context is NOT sent for read.
    // - Pagination is token-based: nextPage cursor + hasMoreResults.
    // - Filter parameters mirror swagger v1.716; expand call sites as needed.
    public IEnumerable<MoInstance> GetMoInstances(
        string? processKey = null,
        string? processType = null,
        string? statuses = null,
        string? packageId = null,
        string? externalId = null,
        DateTime? startedTimeUtcStart = null,
        DateTime? startedTimeUtcEnd = null,
        int pageSize = 100)
    {
        ...
        while (true)
        {
            ...
            if (!string.IsNullOrEmpty(nextPage)) qs.Append($"&nextPage={Uri.EscapeDataString(nextPage)}");
            if (!string.IsNullOrEmpty(processKey)) qs.Append($"&processKey={Uri.EscapeDataString(processKey)}");
            if (!string.IsNullOrEmpty(processType)) qs.Append($"&processType={Uri.EscapeDataString(processType)}");
            if (!string.IsNullOrEmpty(statuses)) qs.Append($"&statuses={Uri.EscapeDataString(statuses)}");
            if (!string.IsNullOrEmpty(packageId)) qs.Append($"&packageId={Uri.EscapeDataString(packageId)}");
            if (!string.IsNullOrEmpty(externalId)) qs.Append($"&externalId={Uri.EscapeDataString(externalId)}");
            if (startedTimeUtcStart.HasValue) qs.Append($"&startedTimeUtcStart={Uri.EscapeDataString(startedTimeUtcStart.Value.ToUniversalTime().ToString("o"))}");
            if (startedTimeUtcEnd.HasValue) qs.Append($"&startedTimeUtcEnd={Uri.EscapeDataString(startedTimeUtcEnd.Value.ToUniversalTime().ToString("o"))}");

            string url = $"{_base_url}/pims_/api/v1/instances{qs}";
            var request = new HttpRequestMessage(HttpMethod.Get, url);
            ...
            string body = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            MoInstanceListResponse? page = JsonSerializer.Deserialize<MoInstanceListResponse>(body);

            if (page?.instances is null || page.instances.Length == 0) yield break;
            foreach (var inst in page.instances) yield return inst;

            if (!page.hasMoreResults || string.IsNullOrEmpty(page.nextPage)) yield break;
            nextPage = page.nextPage;
        }
    }

    // /v1/processes/summary — list all deployed Maestro process definitions for the
    // tenant, with per-process instance counts by status. The endpoint is unpaginated
    // (returns all in one go) and only filters on processType.
    public IEnumerable<MoProcess> GetMoProcesses(string? processType = null)
    {
        string url = $"{_base_url}/pims_/api/v1/processes/summary";
        if (!string.IsNullOrEmpty(processType)) url += $"?processType={Uri.EscapeDataString(processType)}";
        ...
        string body = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
        MoProcessListResponse? page = JsonSerializer.Deserialize<MoProcessListResponse>(body);
        if (page?.processes is null) yield break;
        foreach (var p in page.processes) yield return p;
    }
