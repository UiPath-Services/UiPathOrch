...
// Compare action catalogs between two folders or Orchestrator instances. Matches by Name and
// compares the catalog configuration (encryption, retention, tags). See Compare-OrchAsset for
// the shared model.
...
public class CompareActionCatalogCmdlet : OrchestratorPSCmdlet
{
    ...
    private static readonly (string Name, Func<TaskCatalog, object?> Get)[] Comparators =
    [
        ("Description", c => c.Description),
        ("Encrypted", c => c.Encrypted),
        ("RetentionAction", c => c.RetentionAction),
        ("RetentionPeriod", c => c.RetentionPeriod),
        ("RetentionBucketName", c => c.RetentionBucketName),
        ("Tags", c => EntityComparison.NormalizeTags(c.Tags)),
    ];
    ...
    protected override void ProcessRecord()
    {
        ...
        FolderCompare.Run<TaskCatalog>(
            ...
            only,
            (drive, folder) => drive.ActionCatalogs.Get(folder),
            c => c?.Name,
            c => c!.GetPSPath(),
            Comparators,
