    ...
    protected override void ProcessRecord()
    {
        ...
        foreach (var (_, srcFolder) in srcDrivesFolders.WithCancellation(cancelHandler.Token))
        {
            try
            {
                ...
                var srcEntities = srcDrive.TestSets.Get(srcFolder).FilterByNames(b => b?.Name, Name);
            ...
            catch (Exception ex)
            {
                ...
                WriteError(new ErrorRecord(new OrchException(target, ex), "CopyTestSetError", ErrorCategory.InvalidOperation, dstFolder));
            }
        }
    }
}
