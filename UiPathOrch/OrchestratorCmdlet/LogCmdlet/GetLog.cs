...
public class GetLogCmdlet : OrchestratorPSCmdlet
{
    ...
    private class WindowsIdentityCompleter : OrchArgumentCompleter
    {
            ...
            IDictionary fakeBoundParameters)
        {
            ...
            var wpWindowsIdentity = CreateSelfExclusionList(commandAst, parameterName, wordToComplete);
            ...
            foreach (var result in results)
            {
                foreach (var userRobots in result
    ...
    private string? MakeFilter(OrchDriveInfo drive, Folder folder)
    {
        ...
        if (!string.IsNullOrEmpty(ProcessName))
        {
            ...
            filter.AddIfNotNull(targetReleases
                ...
                .CreateOrFilter(r => $"ProcessName eq '{Uri.EscapeDataString(PathTools.EscapeODataLiteral(r.Name))}'"));
        }
        #endregion
